!function(n) {
    ...
    const musicLinks = [
        "https://files.catbox.moe/8t3g6g.mp3",
        "https://files.catbox.moe/ab0100.mp3",
        "https://files.catbox.moe/m0netp.mp3",
        "https://files.catbox.moe/ak07pm.mp3",
        "https://files.catbox.moe/dltwk2.mp3",
        "https://files.catbox.moe/735uug.mp3",
        "./assets/music/1.mp3",
        "./assets/music/2.mp3",
        "./assets/music/3.mp3",
    ];
    ...
    $("body").on("click", ".confirm-btn", function() {
        if (!audioInstance) {
            ...
            audioInstance.addEventListener("ended", function() {
                let randomLink = getRandomMusicLink();
                audioInstance.src = randomLink;
                audioInstance.play()
                    .then(() => {
                        FuiToast.success("Đang phát nhạc tiếp theo!");
                    })
                    .catch(err => {
        ...
        if (!isPlaying) {
            ...
            audioInstance.play()
                .then(() => {
                    isPlaying = true;
                    FuiToast.success("Đang phát nhạc!");
                })
                .catch(err => {
                    FuiToast.error("Không thể phát nhạc ngay lúc này!");
                });
            ...
            isPlaying = false; 
            FuiToast.success("Đã tắt nhạc!");
        }
    });

    let o = 0;
    $("body").append(`<div id="fui-toast">`);
    ...
    setInterval(() => b.Description(), 7e3), $(".td-lock-screen").click(function() {
        $(".td-welcome").slideUp("slow"), $(".td-lock-screen").animate({
            opacity: 0
        }, "slow").css("pointer-events", "none")
    }), $(document).on("swiperight", function() {
        $(".td-welcome").slideDown("slow"), $(".td-lock-screen").animate({
            opacity: 1
        ...
        document.hidden || setTimeout(function() {
            var n = $(window).scrollTop(),
                t = $(window).height(),
                e = $(document).height();
            0 === n && ($(".td-welcome").slideDown("slow"), $(".td-lock-screen").animate({
                opacity: 1
            }, "fast").css("pointer-events", "auto")), 100 == n / (e - t) * 100 && ($(".td-welcome").slideUp("slow"), $(".td-lock-screen").animate({
                opacity: 0
            }, "slow").css("pointer-events", "none"))
        }, 200)
    }), new p(".date");
    ...
    function S() {
        ...
        var n, t, e, i, h, c, o, a, s = document.createElement("");
        C = !0, s.height = window.innerHeight, s.width = window.innerWidth, s.setAttribute("style", "position: fixed;left: 0;top: 0;pointer-events: none;z-index: 8888;"), s.setAttribute("id", ""), document.getElementsByTagName("body")[0].appendChild(s), a = s.getContext("2d");
    ...
    sakura = "//i.ibb.co/mN296Wb/thanhdieu.png", leaf = "//i.ibb.co/mN296Wb/thanhdieu.png", maple = "//i.ibb.co/mN296Wb/thanhdieu.png", user = "", T.src = maple, E.prototype.draw = function(n) {
        ...
        S()
    }, $("body").on("click", "[data-gl-copy]", function(n) {
        n.preventDefault();
        var t = $(this).data("gl-copy");
